...
// BitgetMessageHandler bitget消息处理器
type BitGetMessageHandler struct {
	// 配置和认证
...
// NewBitGetMessageHandler 创建新的bitget消息处理器
func NewBitGetMessageHandler(config *config.CexExchangeConfig, needLogin bool) *BitGetMessageHandler {
	handler := &BitGetMessageHandler{
		Config:       config,
		NeedLogin:    needLogin,
		LoginStatus:  false,
...
func (h *BitGetMessageHandler) SetWebSocketClient(client *ws.GenericWebSocketClient) {
	h.wsClient = client
}

// SetListeners 设置消息监听器
func (h *BitGetMessageHandler) SetListeners(msgListener OnReceive, errorListener OnReceive) {
	h.Listener = msgListener
	h.ErrorListener = errorListener
}

// HandleMessage 处理普通消息
func (h *BitGetMessageHandler) HandleMessage(message string) error {
...
// HandleError 处理错误消息
func (h *BitGetMessageHandler) HandleError(message string) error {
...
func (h *BitGetMessageHandler) HandleSpecialMessage(message string) (handled bool, err error) {
	if message == "pong" {
...
func (h *BitGetMessageHandler) handleLoginResponse(message string) error {
...
func (h *BitGetMessageHandler) handleDataMessage(message string, jsonMap map[string]interface{}) error {
...
func (h *BitGetMessageHandler) handleOtherMessage(message string) error {
...
func (h *BitGetMessageHandler) getListener(argJson interface{}) OnReceive {
	if argJson == nil {
		return h.Listener
	}
...
// IsLoggedIn 检查是否已登录
func (h *BitGetMessageHandler) IsLoggedIn() bool {
	h.mu.RLock()
	defer h.mu.RUnlock()
...
func (h *BitGetMessageHandler) AddSubscription(req model.SubscribeReq, listener OnReceive) {
	h.mu.Lock()
	defer h.mu.Unlock()
...
func (h *BitGetMessageHandler) RemoveSubscription(req model.SubscribeReq) {
	h.mu.Lock()
	defer h.mu.Unlock()
...
// bitgetWebSocketClient Okx WebSocket客户端
type BitGetWebSocketClient struct {
	*ws.GenericWebSocketClient
	MessageHandler *BitGetMessageHandler
}

// NewBitGetWebSocketClient 创建新的Okx WebSocket客户端
func NewBitGetWebSocketClient(config *config.CexExchangeConfig, needLogin bool) *BitGetWebSocketClient {
	// 创建WebSocket配置
	...
	// 创建Okx消息处理器
	messageHandler := NewBitGetMessageHandler(config, needLogin)
...
// Login 登录
func (h *BitGetMessageHandler) Login() error {
	if h.wsClient == nil {
...
// Subscribe 订阅
func (c *BitGetWebSocketClient) Subscribe(req model.SubscribeReq, listener OnReceive) error {
	// 添加到订阅映射
...
// SubscribeList 订阅列表
func (c *BitGetWebSocketClient) SubscribeList(reqs []model.SubscribeReq, listener OnReceive) error {
...
// Unsubscribe 取消订阅
func (c *BitGetWebSocketClient) Unsubscribe(req model.SubscribeReq) error {
	// 从订阅映射中移除
...
func (c *BitGetWebSocketClient) UnsubscribeList(req []model.SubscribeReq) error {
	var args []interface{}
	for _, req := range req {
		// 从订阅映射中移除
		c.MessageHandler.RemoveSubscription(req)
...
// SetListeners 设置监听器
func (c *BitGetWebSocketClient) SetListeners(msgListener OnReceive, errorListener OnReceive) {
	c.MessageHandler.SetListeners(msgListener, errorListener)
}

// IsLoggedIn 检查是否已登录
func (c *BitGetWebSocketClient) IsLoggedIn() bool {
