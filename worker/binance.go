package worker

import (
	"context"
	"errors"
	"fmt"
	"github.com/339-Labs/exchange-market/common/maps"
	"github.com/339-Labs/exchange-market/common/tasks"
	"github.com/ethereum/go-ethereum/log"
	"time"
)

type BinanceTask struct {
	spotPriceMap    *maps.PriceMap
	featurePriceMap *maps.PriceMap
	resourceCtx     context.Context

	resourceCancel context.CancelFunc
	tasks          tasks.Group
	ticker         *time.Ticker
}

func NewBinanceTask(cfg *context.Context, shutdown context.CancelCauseFunc, duration time.Duration, spotPriceMap *maps.PriceMap, featurePriceMap *maps.PriceMap) (*BinanceTask, error) {
	resCtx, resCancel := context.WithCancel(context.Background())
	return &BinanceTask{
		resourceCtx:    resCtx,
		resourceCancel: resCancel,
		tasks: tasks.Group{HandleCrit: func(err error) {
			shutdown(fmt.Errorf("bn ws error "))
		}},
		ticker:          time.NewTicker(duration),
		spotPriceMap:    spotPriceMap,
		featurePriceMap: featurePriceMap,
	}, nil
}

func (t *BinanceTask) Start() error {
	log.Info("bn task started")
	t.tasks.Go(func() error {
		for {

			select {

			case <-t.ticker.C:
				// todo  bn ws data handler, spot and feature

			case <-t.resourceCtx.Done():
				log.Info("stop bn task in work")
				return nil

			}

		}
	})
	return nil
}

func (t *BinanceTask) Close() error {
	var result error
	t.resourceCancel()
	t.ticker.Stop()
	log.Info("bn task stopped")
	if err := t.tasks.Wait(); err != nil {
		result = errors.Join(result, fmt.Errorf("bn ws task wait error"))
	}
	log.Info("bn task stopped success")
	return nil
}
